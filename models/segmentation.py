...
class Modelsegm(nn.Module):
    def __init__(self, model, mask_head="smallconv", freeze_vistr=False):
        super.__init__()
        self.model = model
        ...
        hidden_dim, nheads = model.transformer.d_model, model.transformer.nhead
        ...
        if isinstance(model.backbone[0], TimmBackbone):
            channels = model.backbone[0].body.feature_info.channels()[::-1]
    ...
    def forward(self,  samples: NestedTensor, captions):
        ...
        query_embed = self.model.query_embed.weight
        src_proj = self.model.input_proj(src)
        ...
        mask = mask.reshape(bs_f, self.model.num_frames, s_h*s_w)
        pos = pos[-1].permute(0,2,1,3,4).flatten(-2)


        memory_cache = self.model.transformer(
            src_proj,
            mask,
            query_embed,
            pos,
        ...
        memory = memory_cache["memory"][: -len(memory_cache["text_memory"])].permute(1, 2, 0).view_as(src_proj)
