...
class Model(nn.Module):
    def __init__(
        ...
        self.query_embed_new = nn.Embedding(num_queries+2, hidden_dim)
        ...
        self.start_pred_embed = nn.Linear(hidden_dim, 1)
        self.end_pred_embed = nn.Linear(hidden_dim, 1)
...
###################
    ...
    def forward(self, samples: NestedTensor, captions, encode_and_save=True, memory_cache=None):
        ...
        # hs = self.transformer(src_proj, mask, self.query_embed.weight, pos)[0]
            ...
            out = {}
            outputs_start = self.start_pred_embed(hs[:, :, -2, :])
            outputs_end = self.start_pred_embed(hs[:, :, -1, :])
            hs = hs[:, :, :-2, :]
            ...
            out.update(
                {
                    ...
                    # "pred_starts": outputs_start[-1],
                    # "pred_ends": outputs_end[-1],
                }
            )
...
class SetCriterion(nn.Module):
    ...
    def loss_temporal_labels(self, outputs, targets, positive_map, indices, num_boxes):
        "temporal grounding loss"
        pred_starts = outputs['pred_starts']
        pred_ends = outputs['pred_ends']

        target_temporal_gt = torch.cat([t['temporal_gt'].unsqueeze(0) for t in targets])

        target_starts, target_ends = target_temporal_gt.unbind(-1)

# #######################
#         # 加入one shot
# #######################
#         if target_starts[0][1].cpu().item() == -1:
#             loss_l1 = F.l1_loss(pred_ends, target_ends)
#         else:
#             loss_l1 = F.l1_loss(pred_starts, target_starts)
        loss_l1 = F.l1_loss(pred_starts, target_starts)+F.l1_loss(pred_ends, target_ends)
    ...
    def get_loss(self, loss, outputs, targets, positive_map, indices, num_boxes, **kwargs):
        loss_map = {
            ...
            "temporal_labels": self.loss_temporal_labels,
        }
...
def build(args):
    ...
    weight_dict = {'loss_ce': args.ce_loss_coef, 'loss_bbox': args.bbox_loss_coef, 'loss_l1': args.l1_loss_coef}
    ...
    losses = ['labels', 'boxes', 'cardinality']#, 'temporal_labels']
    if args.masks:
        losses += ["masks"]
    if args.predict_final:
        losses += ["isfinal"]
